...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    /*
     * Format response array message
     */
    protected function format_response_array($msg = NULL, $status = NULL, $data = array()){
	...
	function getRecordsByOffset($table,$where=array(),$select="",$orderBy=null,$start=0,$offset="")
  {
	...
	function getUserIdByMobile($mobile){
		...
		$this->db->where('roleId',USER_ROLL);
		...
		function bloodBankName($id){ 
		$this->db->select('b_bankName');
		$this->db->where('b_bankId', $id);
	    $query = $this->db->get(TBL_BLOOD_BANKS);
		if (!$query)
		{
		     $this->throwException($query);
		}else{
